...
LianLiUniHubALController::LianLiUniHubALController(hid_device* dev_handle, const char* path, unsigned short pid, std::string dev_name)
{
    ...
    for(unsigned int i = 0; i < UNIHUB_AL_CHANNEL_COUNT; i++)
    {
        cached_fan_count[i] = UINT_MAX;
    }
}
...
void LianLiUniHubALController::SetChannelLEDs(unsigned char channel, RGBColor * colors, unsigned int num_colors, float brightness)
{
    ...
    /*---------------------------------------------------------*\
    | Send fan LED data                                         |
    | Only send StartAction when fan count changes to avoid     |
    | redundant packets on every frame during effects playback  |
    \*---------------------------------------------------------*/
    unsigned int current_fan_count = (unsigned int)(fan_idx + 1);

    if(cached_fan_count[channel] != current_fan_count)
    {
        SendStartAction
        (
            channel,            // Current channel
            current_fan_count   // Number of fans
        );
        cached_fan_count[channel] = current_fan_count;
    }
